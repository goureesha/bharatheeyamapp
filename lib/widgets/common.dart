  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [kPurple1, kPurple2], begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        boxShadow: [BoxShadow(color: kPurple2.withOpacity(0.3), blurRadius: 15, offset: const Offset(0, 4))],
        border: const Border(bottom: BorderSide(color: Color(0xFFF6D365), width: 4)),
      ),
      child: const Center(
        child: Text('ಭಾರತೀಯಮ್',
          style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w800, letterSpacing: 1.2)),
      ),
    );
  }
}
...
class SectionTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Text(text, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: color ?? const Color(0xFF2B6CB0))),
    );
  }
}

TextStyle kn({double size = 14, FontWeight fw = FontWeight.normal, Color? color}) =>
    TextStyle(fontSize: size, fontWeight: fw, color: color);
