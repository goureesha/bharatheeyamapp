...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  static List<String> get _tabs {
    switch (AppLocale.current) {
      case 'hi': return ['कुंडली', 'स्फुट', 'आरूढ', 'दशा', 'पंचांग', 'भाव', 'ग्रह षड्वर्ग', 'षड्बल', 'अष्टक', 'टिप्पणी', 'पत्रिका'];
      case 'ta': return ['ஜாதகம்', 'ஸ்புடம்', 'ஆரூடம்', 'தசை', 'பஞ்சாங்கம்', 'பாவம்', 'ஷட்வர்கம்', 'ஷட்பலம்', 'அஷ்டகம்', 'குறிப்பு', 'பத்ரிகை'];
      case 'te': return ['కుండలి', 'స్ఫుటం', 'ఆరూఢం', 'దశ', 'పంచాంగం', 'భావం', 'షడ్వర్గం', 'షడ్బలం', 'అష్టకం', 'గమనికలు', 'పత్రిక'];
      case 'ml': return ['ജാതകം', 'സ്ഫുടം', 'ആരൂഢം', 'ദശ', 'പഞ്ചാംഗം', 'ഭാവം', 'ഷഡ്വർഗം', 'ഷഡ്ബലം', 'അഷ്ടകം', 'കുറിപ്പുകൾ', 'പത്രിക'];
      default: return ['ಕುಂಡಲಿ', 'ಸ್ಫುಟ', 'ಆರೂಢ', 'ದಶ', 'ಪಂಚಾಂಗ', 'ಭಾವ', 'ಗ್ರಹ ಷಡ್ವರ್ಗ', 'ಷಡ್ಬಲ', 'ಅಷ್ಟಕ', 'ಟಿಪ್ಪಣಿ', 'ಪತ್ರಿಕೆ'];
    }
  }
            ...
            children: [
              ...
              AppCard(
                ...
                child: Column(children: [
                  ...
                  _tableRow([AppLocale.l('tithiLabel'), '${trAll(pan.tithi)}${pan.tithiShesha.isNotEmpty ? '\n${AppLocale.l('sheshaGhati')}: ${pan.tithiShesha} / ${pan.tithiParama} ${AppLocale.l('ghatiUnit')}' : ''}']),
                  _tableRow([AppLocale.l('chandraNakshatra'), () { final moonPada = r.planets['ಚಂದ್ರ']?.pada; final fallback = (pan.nakPercent * 4).floor() + 1; final p = moonPada ?? (fallback < 1 ? 1 : fallback > 4 ? 4 : fallback); return '${trAll(pan.nakshatra)} - ${AppLocale.l('padaLabel')} $p${pan.shesha.isNotEmpty ? '\n${AppLocale.l('sheshaGhati')}: ${pan.shesha} / ${pan.paramaGhati} ${AppLocale.l('ghatiUnit')}' : ''}'; }()]),
                  _tableRow([AppLocale.l('yogaLabel'), '${trAll(pan.yoga)}${pan.yogaShesha.isNotEmpty ? '\n${AppLocale.l('sheshaGhati')}: ${pan.yogaShesha} / ${pan.yogaParama} ${AppLocale.l('ghatiUnit')}' : ''}']),
                  _tableRow([AppLocale.l('karanaLabel'), '${trAll(pan.karana)}${pan.karanaShesha.isNotEmpty ? '\n${AppLocale.l('sheshaGhati')}: ${pan.karanaShesha} / ${pan.karanaParama} ${AppLocale.l('ghatiUnit')}' : ''}']),
